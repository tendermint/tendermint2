// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.28.1
// 	protoc        v3.21.6
// source: ed25519.proto

package pb

import (
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	reflect "reflect"
	sync "sync"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

// messages
type PubKeyEd25519 struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Value []byte `protobuf:"bytes,1,opt,name=Value,proto3" json:"Value,omitempty"`
}

func (x *PubKeyEd25519) Reset() {
	*x = PubKeyEd25519{}
	if protoimpl.UnsafeEnabled {
		mi := &file_ed25519_proto_msgTypes[0]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *PubKeyEd25519) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*PubKeyEd25519) ProtoMessage() {}

func (x *PubKeyEd25519) ProtoReflect() protoreflect.Message {
	mi := &file_ed25519_proto_msgTypes[0]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use PubKeyEd25519.ProtoReflect.Descriptor instead.
func (*PubKeyEd25519) Descriptor() ([]byte, []int) {
	return file_ed25519_proto_rawDescGZIP(), []int{0}
}

func (x *PubKeyEd25519) GetValue() []byte {
	if x != nil {
		return x.Value
	}
	return nil
}

type PrivKeyEd25519 struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Value []byte `protobuf:"bytes,1,opt,name=Value,proto3" json:"Value,omitempty"`
}

func (x *PrivKeyEd25519) Reset() {
	*x = PrivKeyEd25519{}
	if protoimpl.UnsafeEnabled {
		mi := &file_ed25519_proto_msgTypes[1]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *PrivKeyEd25519) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*PrivKeyEd25519) ProtoMessage() {}

func (x *PrivKeyEd25519) ProtoReflect() protoreflect.Message {
	mi := &file_ed25519_proto_msgTypes[1]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use PrivKeyEd25519.ProtoReflect.Descriptor instead.
func (*PrivKeyEd25519) Descriptor() ([]byte, []int) {
	return file_ed25519_proto_rawDescGZIP(), []int{1}
}

func (x *PrivKeyEd25519) GetValue() []byte {
	if x != nil {
		return x.Value
	}
	return nil
}

var File_ed25519_proto protoreflect.FileDescriptor

var file_ed25519_proto_rawDesc = []byte{
	,
}

var (
	file_ed25519_proto_rawDescOnce sync.Once
	file_ed25519_proto_rawDescData = file_ed25519_proto_rawDesc
)

func file_ed25519_proto_rawDescGZIP() []byte {
	file_ed25519_proto_rawDescOnce.Do(func() {
		file_ed25519_proto_rawDescData = protoimpl.X.CompressGZIP(file_ed25519_proto_rawDescData)
	})
	return file_ed25519_proto_rawDescData
}

var file_ed25519_proto_msgTypes = make([]protoimpl.MessageInfo, 2)
var file_ed25519_proto_goTypes = []interface{}{
	(*PubKeyEd25519)(nil),  // 0: tm.PubKeyEd25519
	(*PrivKeyEd25519)(nil), // 1: tm.PrivKeyEd25519
}
var file_ed25519_proto_depIdxs = []int32{
	0, // [0:0] is the sub-list for method output_type
	0, // [0:0] is the sub-list for method input_type
	0, // [0:0] is the sub-list for extension type_name
	0, // [0:0] is the sub-list for extension extendee
	0, // [0:0] is the sub-list for field type_name
}

func init() { file_ed25519_proto_init() }
func file_ed25519_proto_init() {
	if File_ed25519_proto != nil {
		return
	}
	if !protoimpl.UnsafeEnabled {
		file_ed25519_proto_msgTypes[0].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*PubKeyEd25519); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_ed25519_proto_msgTypes[1].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*PrivKeyEd25519); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
	}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: file_ed25519_proto_rawDesc,
			NumEnums:      0,
			NumMessages:   2,
			NumExtensions: 0,
			NumServices:   0,
		},
		GoTypes:           file_ed25519_proto_goTypes,
		DependencyIndexes: file_ed25519_proto_depIdxs,
		MessageInfos:      file_ed25519_proto_msgTypes,
	}.Build()
	File_ed25519_proto = out.File
	file_ed25519_proto_rawDesc = nil
	file_ed25519_proto_goTypes = nil
	file_ed25519_proto_depIdxs = nil
}
