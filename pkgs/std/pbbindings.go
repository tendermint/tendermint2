...
func IsBaseAccountReprEmpty(goor BaseAccount) (empty bool) {
	{
		...
		{
			goor, err := goor.Coins.MarshalAmino()
			if err != nil {
				return false
			}
			if goor != string("") {
		...
		{
			if goor.AccountNumber != uint64(0) {
				return false
			}
		}
		{
			if goor.Sequence != uint64(0) {
				return false
			}
		}
	}
...
func (goo InvalidCoinsError) ToPBMessage(cdc *amino.Codec) (msg proto.Message, err error) {
	...
	{
		if IsInvalidCoinsErrorReprEmpty(goo) {
			var pbov *stdpb.InvalidCoinsError
			msg = pbov
			return
		}
...
func (goo InvalidGasWantedError) ToPBMessage(cdc *amino.Codec) (msg proto.Message, err error) {
	...
	{
		if IsInvalidGasWantedErrorReprEmpty(goo) {
			var pbov *stdpb.InvalidGasWantedError
			msg = pbov
			return
		}
